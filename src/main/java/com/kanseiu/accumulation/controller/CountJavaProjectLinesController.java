...
public class CountJavaProjectLinesController {
    ...
    private static final String SRC_DIR_PATTERN = ".*/src";

    private static final String SRC_DIR_PATTERN1 = ".*/src/.*";
    ...
    public static String scan(File f, int lineNumber, int fileNumber) throws IOException {
        ...
        File[] lf = f.listFiles();
        // 如果为空则结束这次方法。避免空指针异常
        if (Objects.isNull(lf)) return null;
        // 循环遍历lf中的每个File对象
        for (File f1 : lf) {
            ...
            if (f1.isDirectory()) {
                ...
                if (f1.getName().endsWith(".java") || f1.getName().endsWith(".xml")) {
                    fileNum++;
                    lineNum = countLineNumsAndFileNums(f1, lineNum);
                }
            }
        }
        log.info("file = {}, lineNum = {}, fileNum = {}", f.getAbsolutePath(), lineNum, fileNum);
        return lineNum + "," + fileNum;
    }

    private static int countLineNumsAndFileNums(File f1, int lineNum){
        //创建当前文件的对象
